...
import com.sun.chat_04.ui.signup.SignUpPresenter
...
class LoginActivity : AppCompatActivity(), View.OnClickListener
    ...
    private fun initComponents() {
        ...
        presenter = LoginPresenter(UserRepository(UserRemoteDataSource(auth, firebaseDatabase)), auth, this)
    ...
    private fun checkValidateLogin(isButtonClicked: Boolean) {
        val email = editEmailLogin.text.toString()
        val password = editPassLogin.text.toString()
        if (::presenter.isInitialized) {
            presenter.checkValidateLogin(email, password, isButtonClicked)
        }
    }
    ...
    override fun validLogin(isButtonClicked: Boolean) {
